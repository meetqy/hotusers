import { Button, Image } from "@nextui-org/react";
import { Icon } from "@iconify/react";
import { strapi } from "~/strapi/api";
import { notFound } from "next/navigation";

const getUserBySlug = async (slug: string, locale = "en") => {
  const res = await strapi.GET("/accounts", {
    params: {
      // @ts-ignore
      query: {
        locale,
        filters: {
          // @ts-ignore
          slug: { $eq: slug },
        },
        // @ts-ignore
        "populate[tags]": true,
        "populate[xhs]": true,
        "populate[twitter]": true,
        "populate[github]": true,
        "populate[douyin]": true,
        "populate[youtube]": true,
        "populate[bilibili]": true,
        "populate[jike]": true,
        "populate[instagram]": true,
        "populate[weibo]": true,
        "populate[avatar][fields][0]": "url",
      },
    },
  });

  const data = res.data?.data?.[0];
  if (!data) return notFound();

  return data;
};

const Scoicals = (props: { [key in string]: string | undefined }) => {
  return (
    <>
      {props.instagram && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">instagram</span>
          <Icon icon={"mdi:instagram"} className="h-10 w-10" />
        </Button>
      )}

      {props.weibo && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">weibo</span>
          <Icon icon={"mdi:sina-weibo"} className="h-10 w-10" />
        </Button>
      )}

      {props.twitter && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">x | twitter</span>
          <Icon icon={"mdi:twitter"} className="h-10 w-10" />
        </Button>
      )}

      {props.github && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">github</span>
          <Icon icon={"mdi:github"} className="h-10 w-10" />
        </Button>
      )}

      {props.youtube && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">youtube</span>
          <Icon icon={"mdi:youtube"} className="h-10 w-10" />
        </Button>
      )}

      {props.douyin && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">douyin</span>
          <Icon icon={"fa6-brands:tiktok"} className="h-10 w-10" />
        </Button>
      )}

      {props.bilibili && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">bilibili</span>
          <Icon icon={"ant-design:bilibili-outlined"} className="h-10 w-10" />
        </Button>
      )}

      {props.xhs && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">小红书</span>
          <Image src="/logos/xhs.png" alt="xhs scoical link" className="w-16" />
        </Button>
      )}

      {props.jike && (
        <Button
          className="h-16 w-full justify-between px-8 xl:h-20"
          radius="full"
          variant="bordered"
          color="warning"
        >
          <span className="text-xl font-medium">即刻</span>
          <Image
            src="data:image/png;base64,"
            alt="jike scoical link"
            className="w-12"
          />
        </Button>
      )}
    </>
  );
};

export default async function Page({
  params: { slug, locale },
}: {
  params: { slug: string; locale: string };
}) {
  const data = await getUserBySlug(slug, locale);

  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center p-4 xl:px-0">
      <section className="grid flex-1 grid-cols-1 overflow-hidden rounded-medium bg-content1 shadow-medium md:grid-cols-5 lg:grid-cols-2">
        {/* left */}
        <div className="col-span-2 flex flex-col items-center justify-center gap-y-8 bg-content2 p-5 text-center md:p-10 lg:col-span-1 lg:gap-y-12 lg:p-20">
          <Image
            src={data.attributes?.avatar?.data?.attributes?.url}
            className="aspect-square w-32 lg:w-40 xl:w-44"
            radius="full"
            alt="Profile picture"
          />

          <div>
            <p className="font-serif text-3xl font-bold text-default-600 md:text-4xl lg:text-5xl xl:text-6xl">
              {data.attributes?.nickname}
            </p>
            <p className="mt-4 text-large text-default-400 xl:text-xl">
              {data.attributes?.bio}
            </p>
          </div>
        </div>

        {/* right */}
        <div className="col-span-3 flex flex-col items-center justify-center gap-5 p-5 md:p-10 lg:col-span-1 xl:p-20">
          <Scoicals
            github={data.attributes?.github?.link}
            twitter={data.attributes?.twitter?.link}
            instagram={data.attributes?.instagram?.link}
            weibo={data.attributes?.weibo?.link}
            youtube={data.attributes?.youtube?.link}
            douyin={data.attributes?.douyin?.link}
            bilibili={data.attributes?.bilibili?.link}
            xhs={data.attributes?.xhs?.link}
            jike={data.attributes?.jike?.link}
          />
        </div>
      </section>
    </div>
  );
}
